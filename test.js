...
function userAge(array, min, max) {
  return array.filter(age => age.age >= min && age.age <= max);
};

// console.log(userAge(users, 20, 30));
...
function getUserWithGender(arrayUsers, gender) { return arrayUsers.filter((user) => user.gender === gender).map(user => user.name) };
...
function getSortedUniqueSkills(array) {
  return array.flatMap(user => user.skills).filter((user, index, arr) => arr.indexOf(user) === index).toSorted((userA, userB) => userA.localeCompare(userB));
}
